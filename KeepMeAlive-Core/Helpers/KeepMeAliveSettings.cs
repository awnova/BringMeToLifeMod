...
namespace KeepMeAlive.Helpers
{
    ...
    internal class KeepMeAliveSettings
    {
        ...
        public static ConfigEntry<bool> TEAM_REVIVE_ENABLED;
        public static ConfigEntry<float> SELF_REVIVE_HOLD_TIME;
        ...
        public static void Init(ConfigFile config)
        {
            ...
            TEAM_REVIVE_ENABLED = config.Bind(
                ...
                "Enable Team Revive",
                true,
                "When enabled, players can revive downed teammates"
            );

            SELF_REVIVE_HOLD_TIME = config.Bind(
                "2. Revival Mechanics",
                "Self Revive Hold Time",
                2f,
                "Seconds to hold the self-revive key before authorization starts"
            );

            TEAM_REVIVE_HOLD_TIME = config.Bind(
                "2. Revival Mechanics",
                "Team Revive Hold Time",
                2f,
                "Seconds to hold team-revive interaction before authorization starts"
            );


            SELF_REVIVE_ANIMATION_DURATION = config.Bind(
                "2. Revival Mechanics",
                "Self Revive Progress Duration",
                10f,
                "Duration (seconds) for self-revive progress before completion"
            );

            TEAMMATE_REVIVE_ANIMATION_DURATION = config.Bind(
                "2. Revival Mechanics",
                "Teammate Revive Progress Duration",
                5f,
            ...
            TEAM_HEAL_NUTRITION_MIN_DEFICIT = config.Bind(
                "4. Team Healing",
                "Nutrition Min Deficit to Display",
                0.5f,
                "Minimum missing Hydration or Energy required to show Medic Nutrition category (0 = show whenever not full)"
            );
