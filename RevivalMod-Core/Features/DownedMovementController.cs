...
{
    /// <summary>
    /// Controls movement restrictions (prone lock, speed limiting, hand clearing) while a player is downed,
    /// and re-attaches movement hooks after revival.
    /// </summary>
    internal static class DownedMovementController
    {
        /// <summary>Force the player into a revivable state: prone, no sprint, empty hands, unhook animations.</summary>
        ...
        /// <summary>Re-subscribe movement/animation event hooks that were stripped when the player went down.</summary>
        ...
        /// <summary>Scale walk speed based on downed settings, or freeze movement during revive.</summary>
        ...
        /// <summary>Enforce prone pose and clear hands (gated to avoid per-frame spam).</summary>
        public static void ApplyDownedMovementRestrictions(Player player, RMPlayer st)
        {
            try
            {
                ...
                {
                    if (player.HandsController != null && player.HandsController.Item != null && !player.HandsController.Item.TemplateId.ToString().Contains("fake"))
                    {
